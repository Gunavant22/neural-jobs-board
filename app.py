...
def safe_fetch_df(query, params=None):
    """🛡️ ULTRA CRASH-PROOF DB READER: Bypasses Pandas Segfaults completely by using Pure Python to fetch data!"""
    conn = psycopg2.connect(DB_URL)
    c = conn.cursor()
    if params:
    ...
    # Pass pure python list to Pandas to avoid C-library conflicts
